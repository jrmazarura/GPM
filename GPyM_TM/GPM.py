...
class GPM:
    def __init__(self, corpus, nTopics, alpha = None, beta = None, gam = None, nTopWords = None, iters = None, N = None):
        ...
        self.prodFactorialCounts = []
        print("corpus=%d, words=%d, K=%d, a=%f, b=%f, g=%f,nTopWords=%d, iters=%d, N=%d" % (len(corpus), len(id2word), nTopics, self.alpha,self.beta, self.gam, self.nTopwords, self.iters, self.N))
        ...
        self.prodFactorialCounts=zeros(len(corpus))
        i=0
        for doc in corpus:
            prod=1
            ...
            for word in doc:
                prod*=gamma(word[1]+1)
            self.prodFactorialCounts[i]=prod
            i=i+1
    ...
    def writeTopicAssignments(self):
        file1 = open("GPM_topicAssignments.txt","w")
        file2 = open("GPM_selectedTopics.txt","w")
        #for i in range(self.numDocuments):
        #[file.write(str(self.topicAssignments[i])+"\n") for i in range(self.numDocuments)]
